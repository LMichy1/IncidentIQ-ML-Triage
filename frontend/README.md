...
- `/history` — paginated incident history, with a "Reviewed" indicator once feedback exists.
...
the backend at `backend/e2e-test.sqlite3`, gitignored). 10 tests across two
files:
...
Both spec files share one backend/database instance in this config, so
